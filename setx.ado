*! version 1.3.1 Beta  October 28, 1999  Michael Tomz
*  (C) Copyright 1999 Michael Tomz, Jason Wittenberg, and Gary King
*  Current version available at http://gking.harvard.edu
*
* SETX: Lists or changes the value of row vector mrt_xc, or mrt_xcH & mrt_xcL
* Option:  [ keepmrt, a programmer's option : do not change mrt globals ]
*          [ nobound, an option that applies only if there are bounds on p]
...
*          e(p), the proportion of 1's in the population
...
version 6.0
   ...
   * HAS USER RUN ESTSIMP PRIOR TO SETX?
   tokenize `"`e(cmd)'"'
   if "`1'" == "relogit" {           /* if the estimation was relogit      */
      tokenize `"`e(p)'"'            /* and there were bounds on p (the    */
      if "`2'" ~= "" {               /* fraction of 1's in the population) */
         setxHL `0'                  /* pass everything to setxHL and exit */
         capture mat list r(mrt_xcL) /* if matrices were passed back...    */
         if _rc==0 { 	             /* ... resave them by posting them to */
            tempname xctmpL xctmpH   /* the return list                    */
            matrix `xctmpL' = r(mrt_xcL)
            matrix `xctmpH' = r(mrt_xcH)
            return matrix mrt_xcL `xctmpL'
            return matrix mrt_xcH `xctmpH'
         }      
         exit			     /* exit this procedure                */
      }
      else { local p `1' }           /* save p -- note: p may not exist    */
   }
   else if "`1'" ~= "estsimp" {
   ...
   * CONSTRUCT SAMPLE DEFINITIONS
   if "`p'" ~= "" {
      tempvar pvar
      qui gen `pvar' = `p'
      qui replace `pvar' = (1-`p') if `e(depvar)'==0
      if "`exp'" ~= "" { 
         local wt [aw`exp'*`pvar']
	   local wtdropp [aw`exp']     /* weight without pvar */
      }
      else if "`noinher'"==""&"`e(wexp)'"~="" {
         local wt [aw`e(wexp)'*`pvar']
         local wtdropp [aw`e(wexp)']  /* weight without pvar */
      }
      else { 
         local wt [aw=`pvar'] 
         local wtdropp                 /* weight without pvar */
      }      
   }
   else {
      if "`exp'" ~= "" { local wt [`weight'`exp'] }
      else if "`noinher'"==""&"`e(wexp)'"~=""{local wt [`e(wtype)'`e(wexp)']}
   }       
   if "`noinher'" == "" {                         
   ...
   * COLLECT VALUE DESCRIPTIONS
   ...
   if "`keepmrt'" == "" {
      ...
      if "`p'"=="" { global mrt_seto `if' `in' `wt', `nocwdel' }
      else { global mrt_seto `if' `in' `wtdropp', `nocwdel' }
   }
